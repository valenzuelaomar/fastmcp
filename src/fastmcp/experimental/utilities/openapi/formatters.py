...
def format_query_parameter_with_style(
    param_value: Any, parameter_info: ParameterInfo
) -> str | dict[str, str] | Any:
    """
    Format a query parameter according to OpenAPI style and explode settings.

    Args:
        param_value: Value to format
        parameter_info: Parameter metadata including style and explode settings

    Returns:
        Formatted value suitable for query parameters
    """
    param_name = parameter_info.name
    style = parameter_info.style or "form"  # Default to form style
    explode = (
        parameter_info.explode if parameter_info.explode is not None else True
    )  # Default to True

    # Handle array values
    if isinstance(param_value, list):
        if explode:
            # With explode=true, return the list so httpx creates repeated parameters
            return param_value
        else:
            # With explode=false, format according to style
            if style == "form":
                ...
                # Unknown style, fallback to comma-delimited
                logger.warning(
                    f"Unknown array style '{style}' for parameter '{param_name}', using form"
                )
                return ",".join(str(v) for v in param_value)

    # Handle object values (deepObject style)
        ...
        if style == "deepObject":
            if explode:
                ...
                for key, value in param_value.items():
                    bracketed_key = f"{param_name}[{key}]"
                    result[bracketed_key] = str(value)
                return result
            else:
                # deepObject with explode=false is not well-defined in OpenAPI spec
                # Fall back to form encoding
                logger.warning(
                    f"deepObject style with explode=false for parameter '{param_name}' is undefined, using form"
                )
                pairs = [f"{k}={v}" for k, v in param_value.items()]
                return ",".join(pairs)
        else:
            # Non-deepObject object, use form encoding
            if explode:
                # Each property becomes a separate parameter with same name
                # This is not typically supported by httpx, so return the dict
                return param_value
            else:
...
__all__ = [
    ...
    "format_query_parameter_with_style",
