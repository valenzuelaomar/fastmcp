...
from .formatters import format_query_parameter_with_style
...
class RequestDirector:
    ...
    def build(
        ...
        # Step 3: Format query parameters according to OpenAPI style/explode
        formatted_query_params = (
            self._format_query_params(route, query_params) if query_params else None
        )
    ...
    def _format_query_params(
        self, route: HTTPRoute, query_params: dict[str, Any]
    ) -> dict[str, Any]:
        """
        Apply OpenAPI style and explode formatting to query parameters.

        Args:
            route: HTTPRoute containing parameter definitions
            query_params: Raw query parameters to format

        Returns:
            Formatted query parameters suitable for httpx
        """
        formatted_params = {}

        # Create a lookup of parameter info by name
        param_info_by_name = {
            param.name: param for param in route.parameters if param.location == "query"
        }

        for param_name, param_value in query_params.items():
            param_info = param_info_by_name.get(param_name)

            if param_info:
                formatted_value = format_query_parameter_with_style(
                    param_value, param_info
                )

                if isinstance(formatted_value, dict):
                    # For deepObject style, merge the keys into formatted_params
                    formatted_params.update(formatted_value)
                else:
                    formatted_params[param_name] = formatted_value
            else:
                # No parameter info found, use value as-is
